...
/*********************************P2TEST.C*******************************
 *
 *    Test program for the MicroPandOS Kernel: phase 2.
 *      v.0.1: December 1, 2023
 *
...
void clockwait_process()
{
    ...
    SYSCALL(SENDMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&clock_wait_payload), 0);
...
void test()
{
    ...
    hp_p1state.status |= IEPBITON | CAUSEINTMASK | TEBITON;

    STST(&hp_p2state);
    hp_p2state.reg_sp = hp_p1state.reg_sp - QPAGE;
    hp_p2state.pc_epc = (memaddr)hp_p2;
    hp_p2state.status |= IEPBITON | CAUSEINTMASK | TEBITON;
    ...
    pFiveSupport.sup_exceptContext[GENERALEXCEPT].status |= IEPBITON | CAUSEINTMASK | TEBITON;
    ...
    pFiveSupport.sup_exceptContext[PGFAULTEXCEPT].status |= IEPBITON | CAUSEINTMASK | TEBITON;
...
void p5gen()
{
    ...
    case RESVINSTR:
        print_term0("privileged instruction\n");
    ...
    // a user mode syscall will cause an ADDRERROR trap
    case ADDRERROR:
...
void p5mm()
{
    ...
    SYSCALL(RECEIVEMESSAGE, (unsigned int)ssi_pcb, (unsigned int)(&pFiveSupAddr), 0);
    ...
    pFiveSupport.sup_exceptState[PGFAULTEXCEPT].status |= KUPBITON;
...
void p5sys()
{
    ...
    {
    case ON:
        print_term0("ERROR: high level SYS call from USER mode process (should be in kernel mode)\n");
        break;
